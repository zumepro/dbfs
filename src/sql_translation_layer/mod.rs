...
impl TranslationLayer {
	...
	pub fn unsafe_write(&mut self, inode: u64, offset: u64, buffer: &[u8]) -> Result<(), Error> {
		...
		println!("buflen: {:?}, start_idx: {}, end_idx: {}", to_write.len(), start_idx, end_idx);
		to_write[0..start_idx.try_into().unwrap()].copy_from_slice(&db_block_data.start_block_data);
		to_write[start_idx.try_into().unwrap()..=(usize::try_from(end_block).unwrap() * 4096 + usize::try_from(end_idx).unwrap())].copy_from_slice(buffer);
